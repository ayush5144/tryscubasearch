...
(function () {
  ...
  // snapshotQuery: add an executed search query to the session buffer (deduped)
  ...
  function buildResultEl(item, apiKey, sendSettleFn) {
  ...
  function init() {
    ...
    // ── sendSettle ────────────────────────────────────────────────────────────
    // Sends only executed search queries for this session. The backend persists
    // a single final settled intent query per session.
    ...
    function renderResults(results) {
      ...
      if (!results || results.length === 0) {
        ...
        for (var i = 0; i < results.length; i++) {
          frag.appendChild(
            buildResultEl(results[i], API_KEY, sendSettle),
          );
        }
      }
    ...
    // ── Search ────────────────────────────────────────────────────────────────
    function doSearch(query, settleSignal) {
      if (destroyed) return;
    ...
    input.addEventListener("keydown", function (e) {
      ...
      if (e.key === "Enter" || e.keyCode === 13) {
        ...
        if (enterQuery.length >= MIN_QUERY_LEN) {
          clearTimeout(settleTimer);
          settleTimer = null;
          clearTimeout(debounceTimer);
          debounceTimer = null;
          if (!queryMeta[enterQuery]) {
            doSearch(enterQuery, "enter");
          } else {
            sendSettle("enter");
          }
        }
    ...
    input.addEventListener("keyup", function (e) {
      ...
      // Search debounce — 100ms, keeps results snappy
    ...
    function onGlobalKeyDown(e) {
      ...
      var enterQuery = input.value.trim();
      if (enterQuery.length < MIN_QUERY_LEN && !lastLogId && sessionBuffer.length === 0) return;
      clearTimeout(settleTimer);
      settleTimer = null;
      if (enterQuery.length >= MIN_QUERY_LEN && !queryMeta[enterQuery]) {
        doSearch(enterQuery, "enter");
      } else {
        sendSettle("enter");
      }
